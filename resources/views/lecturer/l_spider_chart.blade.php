...
<script>
    ...
    const courseData = {
        'CS101': {
            ...
            // CLO and PLO mapping from assessments
            cloPloMapping: {
                'CLO1': ['PLO1', 'PLO2'],
    ...
    document.addEventListener('DOMContentLoaded', function() {
        loadSections();
        // Set default course and load data
        setTimeout(() => {
    ...
    function loadSections() {
        ...
        if (selectedCourse && courseData[selectedCourse]) {
            courseData[selectedCourse].sections.forEach(sectionCode => {
    ...
    function loadStudents() {
        ...
        if (courseCode && sectionCode && courseData[courseCode] && courseData[courseCode].students[sectionCode]) {
            courseData[courseCode].students[sectionCode].forEach(student => {
                const option = document.createElement('option');
    ...
    function loadAttainmentData() {
        ...
        if (studentId === "all" || studentId === "") {
            ...
            attainmentData = calculateCourseAverage(courseCode, sectionCode);
            titleSuffix = sectionCode ? ` - Section ${sectionCode} Average` : " - Course Average";
        } else {
            // Get individual student data
            attainmentData = courseData[courseCode].attainmentData[studentId];
            const student = findStudentById(courseCode, sectionCode, studentId);
    ...
    function calculateCourseAverage(courseCode, sectionCode = null) {
    ...
    function findStudentById(courseCode, sectionCode, studentId) {
        const course = courseData[courseCode];
        if (!course) return null;
